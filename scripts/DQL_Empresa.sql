...
FROM Funcionarios ORDER BY DataNasci;

----- 5) Liste os funcionários como uma lista telefônica, com enedeço e cidade
SELECT DataNasci, PrimeiroNome, SegundoNome, UltimoNome, Fone, Endereco, CEP, Cidade 
FROM Funcionarios ORDER BY PrimeiroNome, SegundoNome, UltimoNome; 
Maiara — Hoje às 00:28
----- 6) Liste o nome completo de todos os funcionários que não tenham segundo nome
SELECT PrimeiroNome, UltimoNome 
FROM Funcionario WHERE SegundoNome = NULL;
...
FROM Funcionarios WHERE Cidade = Itaiutaba and Funcao = Telefonista;

8) Liste os nomes dos funcionários que trabalham no departamento 2
SELECT PrimeiroNome, SegundoNome, UltimoNome 
FROM Funcionarios WHERE CodigoDepartamento = 02;
